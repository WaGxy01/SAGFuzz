...
class ExecutionTraceAnalyzer(OnTheFlyAnalysis):
    ...
    def execute(self, population, engine):
        if not hasattr(self, '_attack_simulator'):
            self._attack_simulator = ReentrancyAttackSimulator(self.env)
    ...
    def execution_function(self, indv, env: FuzzingEnvironment):
        ...
        for transaction_index, test in enumerate(indv.solution):
            ...
            if not result.is_error and transaction["to"] == b'':
                ...
                print(f"[DEBUG] contract deployed at {contract_address}")
        ...
        if contract_address is None and indv.solution:
            contract_address = indv.solution[0]["transaction"]["to"]

        if contract_address is not None:
            print(f"[DEBUG] calling attack simulator, contract={contract_address}")
            confirmed = self._attack_simulator.run(indv, contract_address)
            if confirmed:
                if "reentrancy_confirmed" not in env.results:
                    env.results["reentrancy_confirmed"] = []
                env.results["reentrancy_confirmed"].append({
                    "individual_hash": indv.hash,
